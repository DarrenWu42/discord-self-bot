    ...
    async def help(self, ctx, commandName: str = None):
        ...
        if commandName is not None:
            ...
            if commandName2 is None:
                ...
                embed = discord.Embed(
                    title=f"{commandName2.name.upper()} Command", description="", color=randint(0, 0xffffff))
            ...
            embed = discord.Embed(
                title=f"Help page", description=f"{self.bot.command_prefix}help (commandName), display the help list or the help data for a specific command.", color=randint(0, 0xffffff))
            embed.set_thumbnail(url=f'{self.bot.user.avatar_url}')
            for i in self.bot.commands:
